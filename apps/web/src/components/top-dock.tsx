...
export function TopDock({ onOpenHistory, onToggleHistory, isHistoryOpen }: TopDockProps) {
  ...
  const currentView: 'chat' | 'live' | 'sil' | 'audios' | 'admin' | 'hojas' | 'centinela' | 'mi-memoria' | 'informes' =
    ...
    : path.startsWith('/informes-semanales') ? 'informes'
    : path.startsWith('/sesiones') || path.startsWith('/expediente') ? 'live'
    : 'chat';
  const handleNavigate = (view: 'chat' | 'live' | 'sil' | 'audios' | 'admin' | 'hojas' | 'centinela' | 'mi-memoria' | 'informes') => {
    ...
    else if (view === 'informes') navigate('/informes-semanales');
