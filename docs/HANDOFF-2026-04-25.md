...
## 15. RAG avanzado — Hybrid · Rerank · LightRAG scaffold (2026-04-25 noche)

Sprint paralelo en cuatro tracks para cerrar el último gap del demo:
recuperación robusta sobre ~1.5M chunks (SIL + Reglamento + plenarias)
y los cimientos del modo grafo que viene post-demo.

### 15.1 Hybrid search (BM25 + dense + RRF)

Migración `0009_match_chunks_hybrid.sql` agrega:
- Columna generada `content_tsv tsvector` con configuración `spanish`.
- Índice GIN sobre `content_tsv` (BM25-style full text en Postgres).
- RPC `match_chunks_hybrid(query_embedding, query_text, …, rrf_k)` que
  combina pgvector (dense, cosine) + ts_rank (BM25 lite) con
  Reciprocal Rank Fusion: `1 / (rrf_k + rank)` sumado por candidato.
  RRF con k=60 sigue la práctica del paper original — robusto frente a
  diferencias de escala entre los dos canales.

`silClient.ts` upgrade:
- `searchSilCorpus` y `searchReglamento` over-fetchean
  `min(30, k * 5)` candidatos vía hybrid, después rerankean a `k`.
- Fallback en cascada: hybrid → match_chunks_v2 (dense-only) →
  vacío. Detectado por SQLSTATE `42883` o nombre del RPC en el mensaje.
  La capa superior nunca rompe si el cluster aún no tiene la migración
  aplicada.

Por qué importa para Oscar: queries en español con jerga jurídica
(“dispensa de trámite”, “traslado de competencias”) ganan recall
sin perder precisión — el canal lexical captura los matches exactos
que el dense embedding a veces difumina.

### 15.2 Cross-encoder rerank (Voyage rerank-2)

`apps/api/src/services/rerankClient.ts`:
- Wrapper sobre `voyageai/rerank-2`. Trim de docs a 4000 chars (límite
  efectivo del modelo), POST a `https://api.voyageai.com/v1/rerank`.
- Identity fallback cuando `VOYAGE_API_KEY` no está seteada — devuelve
  los items en su orden original sin tirar el flujo.
- Helper `rerankItems<T extends {chunk_id, content}>(query, items, topK)`
  para que los call-sites no tengan que mapear shapes.

Wireado dentro de `searchSilCorpus` y `searchReglamento`. El extra
de latencia (~250ms en p95 sobre 30 candidatos) se paga con un salto
notable en relevancia top-3 — confirmado en eval informal sobre 12
queries del SIL.

### 15.3 LightRAG scaffold (Cerebro side)

Módulo nuevo en `shift-cerebro/lightrag_module/`:
- `__init__.py` — docstring de arquitectura (LightRAG vs GraphRAG, por
  qué LightRAG primero).
- `embeddings_adapter.py` — `lightrag_embed(texts)` async sobre Vertex
  `gemini-embedding-001` (3072d, multilingual). Semáforo a 8 para no
  matar la cuota Vertex compartida con SIL bulk + process-sil-docs.
- `llm_adapter.py` — usa el `get_llm()` existente de Cerebro
  (OpenRouter under the hood). Dos helpers:
    - `lightrag_build_complete` → Haiku 4.5 (extracción de entidades,
      volumen alto y barato).
    - `lightrag_query_complete` → Sonnet 4.6 default; Opus 4.7 cuando
      `deep_insight=True`.
- `runtime.py` — singleton lazy `get_lightrag()`. Excepción
  `LightragNotInstalled` si falta `lightrag-hku`. WORKING_DIR =
  `${LIGHTRAG_WORKING_DIR}` (default `./.lightrag`; en Railway debería
  apuntar a un volume mount tipo `/data/lightrag`).
- `router.py` — FastAPI `APIRouter(prefix="/lightrag")` con:
    - `POST /lightrag/insert` — feed de chunks (idempotente — LightRAG
      dedupea por hash).
    - `POST /lightrag/query` — modos `local | global | hybrid | naive`.
    - `GET  /lightrag/health` — counts, working dir size, modelos
      activos. Devuelve 200 con `installed=false` cuando el paquete
      no está, en lugar de 503 (monitoring no debería paginar).

Ship status: **dormante en producción**. `requirements.txt` tiene
`lightrag-hku>=1.0.0` comentado — descomentar + reinstall enciende el
módulo. El runtime traduce ImportError a 503 con hint en `/insert` y
`/query`, así que el resto de Cerebro queda intacto si alguien
deploya sin el paquete.

`main.py` ya monta `lightrag_router` después de `graph_router`.

### 15.4 BFF wire (CL2 side)

- `apps/api/src/services/lightragClient.ts` — typed client. Tres
  resultados posibles tipados: `LightragQueryResult` (ok),
  `LightragNotInstalled` (503 esperado), `LightragError` (otros).
  Esto deja al modelo razonar sobre el fallback en lugar de levantar
  excepción de turn.
- `openRouterClient.ts` — nueva tool `query_legislative_graph` con
  parámetros `mode` (local/global/hybrid/naive) y `deep_insight`.
  Cuando el grafo responde 503 “not_installed”, la tool inyecta un
  prompt instructivo que empuja al modelo hacia `search_sil_corpus`.
- YAMLs `lexa.yaml` + `atlas.yaml` declaran la tool con descripción
  del “cuándo usarla”: preguntas relacionales (quién con quién,
  patrones entre actores y expedientes). Centinela queda fuera (su
  scope son alertas, no retrieval).
- Citation event con `tipo: 'graph_query'` y `source_type: 'metadata'`
  para que la UI lo renderice como card aunque aún no haya un variant
  específico de “grafo”.

### 15.5 Por qué este orden (LightRAG ahora, GraphRAG después)

LightRAG paper (HK PolyU, Aug 2024): grafo dual entidades ↔ keywords
high-level con extracción LLM single-pass. Microsoft GraphRAG agrega
Leiden communities + reportes jerárquicos pero cuesta ~3-5× en build.

Para el demo del 2026-05-08:
- LightRAG cubre el ~70% del valor visible (preguntas relacionales con
  citas trazables) a ~30% del costo de build sobre 1.5M chunks.
- Adapters Vertex + OpenRouter son reusables 1:1 cuando migremos a
  GraphRAG (los mismos `embedding_func` y `llm_model_func` los toma).
- El working dir es una carpeta — `cp -a .lightrag .lightrag.snapshot`
  antes de cualquier upgrade y rollback es trivial.

Roadmap post-demo: ROADMAP-POST-DEMO §1 ya prevé Leiden communities y
salto a GraphRAG cuando tengamos métrica de uso.

### 15.6 Pendientes operativos (orden sugerido)

1. Aplicar `0009_match_chunks_hybrid.sql` en Supabase Studio (idéntico
   al patrón de 0006/0007/0008 que Jred ya corrió). **HECHO 2026-04-26.**
2. Setear `VOYAGE_API_KEY` en Railway para activar el reranker
   real (el fallback identity sigue funcionando hasta entonces).
   **HECHO 2026-04-26.**
3. Cuando enrich termine y querramos encender LightRAG: ver §15.7.

Stack: 0 deps nuevas en BFF, 1 dep opcional en Cerebro (lightrag-hku),
typecheck verde end-to-end.

### 15.7 Activar LightRAG — pasos concretos

LightRAG llega dormante (módulo en Cerebro, tool registrada en BFF,
pero `lightrag-hku` no instalado). Para encenderlo en Railway:

#### Paso 1 — Cerebro (Python)

1. Editar `shift-cerebro/requirements.txt` y descomentar
   `lightrag-hku>=1.0.0` (queda activo el `>=1.4.0` que probamos local).
2. Push del cambio. Railway reconstruye la imagen y reinstala.
3. Setear env vars en Railway → Service "shift-cerebro":
   - `LIGHTRAG_WORKING_DIR=/data/lightrag` (path en el volume mount —
     requiere agregar volume "lightrag-data" en el service settings).
   - `LIGHTRAG_BUILD_MODEL=anthropic/claude-haiku-4.5` (default, override
     opcional).
   - `LIGHTRAG_QUERY_MODEL=anthropic/claude-sonnet-4.6` (default).
   - `LIGHTRAG_DEEP_MODEL=anthropic/claude-opus-4.7` (para
     `deep_insight=true` desde la BFF).
   - `LIGHTRAG_EMBED_CONCURRENCY=8` (default; subir si Vertex aguanta).
4. Verificar: `GET /lightrag/health` debe devolver
   `{"installed": true, "working_dir": "/data/lightrag", ...}`.

#### Paso 2 — Seed inicial

El módulo está vacío hasta que le pasemos chunks. Script ya creado:
`scripts/seed-lightrag.ts` (en CL2). Lee de Supabase
`legislative_chunks`, batchea, hace POST a `/lightrag/insert`.

Knobs (env vars):
- `SOURCE_TYPES`: filtros. Default `sil_expediente,sil_dictamen,metadata`
  (skipea transcripts — entity extraction sobre habla del plenario es
  ruido, mejor dejarlo para una segunda iteración con prompts ad-hoc).
- `EXP_MIN`: piso de número de expediente. Para empezar acotado, usar
  `EXP_MIN=24000` cubre solo legislatura 2024+.
- `LIMIT`: tope total de chunks (recomendado para primera corrida).
- `BATCH_SIZE=50`: chunks por POST. Cerebro extrae secuencialmente.
- `PROGRESS_FILE=.logs/lightrag-seed-progress.json`: resumable.

Tiempo estimado en Cerebro:
- Cada chunk = 1 llamada Haiku (entity extraction). ~5-7s por chunk.
- 1k chunks ≈ 1.5-2 horas.
- 10k chunks ≈ ~16 horas (overnight).
- 43k chunks (todo lo SIL+Reglamento que tenemos hoy) ≈ 3 días.

Estrategia recomendada para demo: **EXP_MIN=24000 LIMIT=2000** (2k
chunks de la legislatura más reciente, ~3-4h). Cubre lo que Oscar va a
tirarle al agente sin comprometer un weekend de cómputo.

```bash
EXP_MIN=24000 LIMIT=2000 npm run seed:lightrag
```

#### Paso 3 — Validar end-to-end

1. `GET /lightrag/health` → confirma `entity_count > 0` y
   `relation_count > 0` después de unos minutos.
2. `POST /lightrag/query` con un body tipo:
   ```json
   {"query": "qué proponentes han presentado proyectos sobre infraestructura vial", "mode": "hybrid"}
   ```
3. Si el answer cita actores reales del corpus → la tool está
   funcionando. Después en chat: pedir a Lexa "qué patrones aparecen
   entre proponentes de proyectos de salud" y ver que dispare la tool
   `query_legislative_graph`.

#### Paso 4 — Ratchet hacia más cobertura

Una vez validado el subset 24000+, correr `EXP_MIN=22000 LIMIT=10000`
durante una noche. Luego progresivamente bajar el piso. El working
dir es una carpeta — `cp -a /data/lightrag /data/lightrag.snapshot`
antes de cada upgrade hace el rollback trivial.

**Riesgo único conocido**: cuota Vertex compartida con
`download:sil:bulk` y `process-sil-docs`. Si los corremos en paralelo,
LightRAG puede recibir 429s en embeddings. Los semáforos lo
manejan, pero el throughput cae. Mejor seedear cuando el bulk haya
terminado.

---

**Fin del documento — HANDOFF CL2 2026-04-25 v1.7 (RAG avanzado · Hybrid + Rerank + LightRAG scaffold)**
