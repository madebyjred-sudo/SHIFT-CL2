...
1. Aplicar `0009_match_chunks_hybrid.sql` en Supabase Studio (idéntico
   al patrón de 0006/0007/0008 que Jred ya corrió).
2. Setear `VOYAGE_API_KEY` en Railway para activar el reranker
   real (el fallback identity sigue funcionando hasta entonces).
3. Cuando enrich termine y querramos encender LightRAG:
   - Descomentar `lightrag-hku>=1.0.0` en `shift-cerebro/requirements.txt`.
   - Setear `LIGHTRAG_WORKING_DIR` a un path persistente en Railway.
   - Llamar `POST /lightrag/insert` por lotes con los chunks ya
     embebidos (script de seed pendiente — no bloquea demo).
