...
| Día | Acción | Dueño / cuándo |
|---|---|---|
| **D2** | Correr `npm run backfill:sil:sharepoint` (≤1h) + `npm run backfill:sil:webforms` (≤2h) → ~85k rows en Supabase | Juanma o sesión próxima |
| **D2** | Worker `processSilDocs.ts`: bajar PDFs últimos 12 meses + leyes aprobadas a GCS, parsear, chunkear, embedding Vertex, insert en `legislative_chunks(source_type='sil_*')` | siguiente sesión |
| **D3** | Suite Playwright e2e simulando asambleísta: login → query SIL → deep_insight Opus → click expediente → ver detalle. CI ready. | siguiente sesión |
| **D3** | Agregar `fetch_sil_live` (Playwright on-demand para data fresca) como cuarta tool — usa `silWebFormsClient` directo cuando DB no tiene el expediente | siguiente sesión |
| **D4** | Update DEMO-RUNBOOK Acto 3 con flow SIL + deep_insight; ajustar Q&A esperado | última sesión pre-demo |
...
**Fin del documento — HANDOFF CL2 2026-04-25 v1.2 (SIL integration D1)**
