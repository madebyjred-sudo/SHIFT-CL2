...
const QUERIES = [
  '¿De qué trata el proyecto de ley que adiciona el artículo 144 bis al código penal propuesto por Mayuli Ortega Guzmán?',
];
...
(async () => {
  for (const q of QUERIES) {
    ...
    const hits = (data ?? []) as Array<{ similarity: number; comision: string; content: string, metadata: any }>;
    if (!hits.length) {
      console.error('  NO HITS');
      continue;
    }
    hits.forEach((h, i) => {
      console.log(`  [${i + 1}] sim=${(h.similarity * 100).toFixed(1)}% [${h.comision}] (Exp: ${h.metadata?.sil_expediente_numero}) ${h.content.slice(0, 90)}...`);
    });
  }
  console.log('\nRAG pipeline OK.');
})();
